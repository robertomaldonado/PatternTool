...
void TextParser::generateNumericalCharacters(){
    ...
    for (int i = 0; i < numericalChars.size()+1 ; i++){    
...
//void TextParser::compareFarewell(std::vector<std::string>& toCompare){ //Metodo mejorado de compareCharactersToFile
//
...
double TextParser::getWS(std::string text){ //
    ...
    /////////////////////
    //cout << "Espacios encontrados:" <<number_of_ws << endl;
    return number_of_ws;
}
...
void TextParser::getAverages(){
    ...
    averageVectorCountDec.push_back( getWS(wholeText) );//51 whitespace per sentence
...
void TextParser::compareNumericalChars(std::vector<std::string>& toCompare){ //Metodo mejorado de compareCharactersToFile
   ...
   // cout << "Total de numericos son: " << total <<endl;
    ...
    numericalCharsCountDec[10]= total;
...
void TextParser::compareMinusCharsImproved(std::vector<std::string>& toCompare){ //Metodo mejorado de compareCharactersToFile
    ...
    int nuevoTotal = 0;
    for(int p =0; p < minusChars.size(); p++){
        nuevoTotal += minusCountDec2[p]  ;
    }
    
    
    total = a+b+c+d+e+f+g+h+i+j+k+l+m+n+o+p+q+r+s+t+u+v+w+x+y+z;
   
    //cout << "Total de minusculas son: " << total <<endl;
    //cout << "Total de minusculas mejoradas son: " << nuevoTotal <<endl;
    nuevoTotal = 0;
