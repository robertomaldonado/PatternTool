...
grep "AWL " global.dat | awk '{s+=$4}END{printf "%.0f %.4f\n" ,1 , (s/3)}' > estadisticaGlobal.dat
grep "AWPS " global.dat | awk '{s+=$6}END{printf "%.0f %.4f\n" ,2 , (s/3)}' >> estadisticaGlobal.dat
grep "AWPP " global.dat | awk '{s+=$8}END{printf "%.0f %.4f\n" ,3 , (s/3)}' >> estadisticaGlobal.dat
grep "ADPP " global.dat | awk '{s+=$10}END{printf "%.0f %.4f\n" ,4 , (s/3)}' >> estadisticaGlobal.dat
grep "ACPP " global.dat | awk '{s+=$12}END{printf "%.0f %.4f\n" ,5 , (s/3)}' >> estadisticaGlobal.dat

grep "ACOPP " global.dat | awk '{s+=$14}END{printf "%.0f %.4f\n" ,6 , (s/3)}' >> estadisticaGlobal.dat
grep "ASCPP " global.dat | awk '{s+=$16}END{printf "%.0f %.4f\n" ,7 , (s/3)}' >> estadisticaGlobal.dat
grep "AQPP " global.dat | awk '{s+=$18}END{printf "%.0f %.4f\n" ,8 , (s/3)}' >> estadisticaGlobal.dat
